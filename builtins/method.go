...
func callTypeMethod(typeName, methodName string, s *symbols.SymbolTable, args []interface{}) (interface{}, error) {
				...
				if ft, ok := ftv.(*data.Type); ok {
					...
					if finfo := ft.Get(methodName); finfo != nil {
						...
						if fd, ok := finfo.(data.Function); ok {
							...
							if f, ok := fn.(func(s *symbols.SymbolTable, args []interface{}) (interface{}, error)); ok {
								// 9. Use the entrypoint to call the method
								return f(s, []interface{}{})
							}
